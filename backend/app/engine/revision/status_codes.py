...
The codes themselves are domain value objects
(`domain.enums.status_codes.ReviewCode` / `IssueCode`); this module owns only
their loading and resolution.
"""
...
from ...domain.enums.status_codes import IssueCode, ReviewCode
...
class StatusCodeBook:
    ...
    def from_rows(cls, rows: list[tuple]) -> "StatusCodeBook":
        ...
        Takes plain tuples, so it is testable without a workbook and carries no
        dependency on the Excel layer.
        """
        book = cls()
        for row in rows:
            def cell(i: int) -> str:
