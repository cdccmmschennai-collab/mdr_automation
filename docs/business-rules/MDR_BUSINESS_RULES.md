...
Source workbook: `data/input/current/_20260720-184-Transmittal Log (9) MDR.xlsx`
Reference workbook: `data/reference/working/20260516_184-Transmittal Log (8).xlsx`
Rules workbook: `data/rules/INPUT-KEYWORDS  FOR MDR TOOL.xlsx`
...
| Sheet | Size | Role |
|---|---|---|
| `QatarEnergy-TN` | 22,009 Ã— 39 | Primary document/transmittal log. Header on **row 5**. |
| `TN FROM VENDORS` | 605 Ã— 16 | Vendor submissions. Header on **row 5**. |
| `Status Codes` | 14 Ã— 6 | Review codes (col A/B) + issue codes (col D/E/F). |
| `VENDOR LIST` | 36 Ã— 3 | Vendor â†’ responsible engineer. |

**A `VENDOR DETAILS` sheet does not exist** in either workbook â€” only
`VENDOR LIST`. The brief names it; the data does not have it. Nothing in
Phase 1 depends on it.

### Reference workbook (8)

Contains the same four sheets **plus `QatarEnergy-TN WORKING`** (21,377 Ã— 46) â€”
the manually prepared working file. It is the log sheet with five MDR columns
inserted at **AKâ€“AO**, which pushes the original last three columns to ARâ€“AT:
...
`QatarEnergy-TN` column **X â€” `LATEST/ NOT LATEST`** already holds the manual
...
### RULE 1 â€” Alphabetic revisions rank later than numeric âœ… CONFIRMED
...
### RULE 2 â€” `Z` is an as-built marker, not the 26th letter âœ… CONFIRMED
...
### RULE 3 â€” Cancellation does *not* exclude a row from latest âœ… CONFIRMED
...
### RULE 4 â€” Withdrawn submissions are excluded âœ… CONFIRMED (n=1)
...
`WITHDRAWIN` and its remarks `WITHDRAWN EMAIL â€¦ RETURNED TO TECHNIP`.
...
## 3. Rejected rule â€” document renumbering âŒ TESTED AND REJECTED

431 document groups have **no `L` at all** â€” every row is `NL`. Their profile
...
| Single-row group | 403 / 431 | â€” |

Where remarks exist they read *"DOC NO. UPDATED REF DOC NO. X"*,
*"DOC NO. CHANGED TO NON-DELIVERABLE"*, *"DOC REF NO HAS BEEN CHANGED FROM â€¦"*
â€” the document continued life under a **different number**.

A rule excluding rows with such remarks was implemented and then **removed**,
because the evidence does not support it:

- the remark pattern matches 182 rows â†’ **128 `NL` / 50 `L`**
...
|---|---|---|
| `WORKBOOK_MARKS_GROUP_INACTIVE` | 431 | Out of scope â€” not modelled (Â§3) |
| `STALE_HIGHER_REVISION_UNPROCESSED` | 77 | **Engine** â€” workbook backlog |
| `STALE_DUPLICATE_LATEST_FLAG` | 2 | **Engine** â€” stale flag |
| `GENUINE_CONFLICT` | **0** | â€” |

### On the "adjusted 100%"

This figure measures agreement over rows the engine is accountable for. It
**excludes** the 431 inactive-group rows, where the engine is *not validated* â€”
...
(`VEN-4391-M4TY-2-80-0003`, `4391-5-MS-0043`) â€” both from rows dated
...
| Observation | Count |
|---|---|
| Issue-code casing variants (`AfC`, `Re-AFC`, `RE--IFA`) | 372 rows â€” normalised |
| Rows with `-` or blank `DOCUMENT NO.` | 286 â€” skipped, not documents |
| Distinct `REV` values | 38 (incl. `-`, `Z`, multi-digit to `18`) |
| Duplicate top revision within a group | 2 groups â€” raised as exceptions |
...
No AI/LLM is used anywhere â€” every decision is deterministic and explainable.
...
1. **What does `NL` on a whole document group mean?** (431 groups) Renumbered,
   superseded, non-deliverable â€” or simply never reviewed? This is the single
